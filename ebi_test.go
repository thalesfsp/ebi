...
func TestBulkCreate_indexUpdateDelete(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.NotNil(t, updateOpts)

	// Update the documents with new values.
	updatedDocs := []*TestModel{
		{
			ID:    "1",
			Group: 3, // Updated value.
		},
		{
			ID:    "2",
			Group: 7, // Updated value.
		},
	}

	assert.NoError(
		t, ebi.BulkCreate(context.Background(), updatedDocs, updateOpts),
	)
}
